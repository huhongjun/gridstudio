...
if [[ "$OSTYPE" == "msys" ]]; then
	...
	if [ ! "$(docker ps -a | grep gridstudio)" ]; then
		...
		docker run --name=gridstudio --rm=false -v $WIN_PWD\\grid-app:/home/source -v $WIN_PWD\\grid-app\\proxy\\userdata:/home/userdata -p 9080:8080 -p 4430:4430 huhongjun/gridstudio:latest
	else
	...
	if [ ! "$(docker ps -a | grep gridstudio)" ]; then
		...
		docker run --name=gridstudio --rm=false -v $PWD/grid-app:/home/source -v $PWD/grid-app/proxy/userdata:/home/userdata -p 9080:8080 -p 4430:4430 huhongjun/gridstudio:v1
	else
		echo "gridstudio container detected - starting container - want to do a full restart? Run destroy.sh first."
		docker start gridstudio
	fi
